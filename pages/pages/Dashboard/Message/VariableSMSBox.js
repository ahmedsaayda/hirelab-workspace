...
const VariableSMSBox = ({ candidateId, onSend, defaultBody }) => {
  const {
    state: body,
    setState: setBody,
    undo: undoBody,
    redo: redoBody,
  } = useHistory(defaultBody ?? localStorage["lastSMSMessage_message"] ?? "");
  ...
  return (
    <>
      ...
      <div className="w-full flex items-center">
        ...
        <button
          onClick={async () => {
            if (!body) return message.error("SMS cannot be empty");
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={loading}
        >
          {!loading ? "Send SMS" : <Spin>Send SMS</Spin>}
        </button>
      </div>
    </>
  );
