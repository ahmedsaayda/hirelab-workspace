    ...
    <div className="grid customScrollbar h-[400px] border rounded-lg m-1 p-1 pl-0 overflow-y-scroll  grid-cols-[100px_1fr]  gap-4 shadow-lg">
      ...
      <div className="flex flex-col border-r border-gray-300">
        {timeSlots.map((time, index) => (
          <div
            key={index}
            className="h-16 flex items-center justify-end pr-4 text-sm text-black-900 border-b "
          >
            {time}
          </div>
        ))}
      ...
      <div className="relative border-b flex flex-col  ">
        {filteredSchedule?.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className="absolute left-0  cursor-pointer   text-black-900 rounded-lg shadow-sm  p-2 text-sm"
              style={{
