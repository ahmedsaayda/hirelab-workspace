...
export default function Variant1({ variant, brandData, landingPageData }) {
  ...
  const [titleSize, setTitleSize] = React.useState(60);
  ...
  React.useEffect(() => {
    ...
    let guard = 0;
    while (guard < 120 && getLines() > allowedLines && size > 46) {
      size -= 2;
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontWeight: 700, fontSize: "28px", color: "#ffffff" }}>{brandName}</span>
      </div>

      {/* Top hero image */}
      <div style={{ position: "absolute", left: 0, top: 0, width: "1080px", height: "700px", overflow: "hidden" }}>
        <img
          src={heroImage}
          alt="about company"
          style={{
      ...
      {/* Bottom panel */}
      <div style={{ position: "absolute", left: 0, top: 580, width: 1080, height: 500 }}>
        <div
          style={{
            position: "absolute",
            inset: 0,
        ...
        {/* Divider */}
        <div style={{ position: "absolute", left: 100, top: 181, right: 100, height: 1, backgroundColor: "rgba(255,255,255,0.6)" }} />
