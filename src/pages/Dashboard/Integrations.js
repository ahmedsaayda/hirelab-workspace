    ...
    <div className="max-w-4xl mx-auto py-8 px-4">
      ...
      {metaStatus?.connected && (
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mt-6">
          <div className="p-6 border-b border-gray-100">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500">
                  Send WhatsApp messages to candidates directly from HireLab.
                </p>
              </div>
              {whatsappLoading ? (
                <Spin size="small" />
          ...
          <div className="p-6">
            {whatsappAssetsLoading ? (
              <div className="flex items-center justify-center py-8">
                <Spin />
              </div>
            ) : whatsappAssets.whatsappAccounts.length === 0 ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-500 mb-4 max-w-md mx-auto">
                  To send WhatsApp messages, you need a WhatsApp Business Account connected to your Meta Business account.
                </p>
                <a
              ...
              <div className="space-y-6">
                ...
                {whatsappStatus?.connected && (
                  <div className="flex items-center gap-3 p-4 bg-green-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-green-800">
                        WhatsApp configured with phone number:{" "}
                        <span className="font-medium">
                          {whatsappStatus.displayPhone || whatsappStatus.phoneNumberId}
                        </span>
                      </p>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <Select
                    className="w-full"
                    placeholder="Select a WhatsApp phone number"
                    loading={whatsappAssetsLoading}
                    value={selectedPhoneNumber}
                    onChange={(val, option) => {
                      setSelectedPhoneNumber(val);
                      if (option?.wabaId) {
                        setSelectedWaba(option.wabaId);
                      }
                    }}
                  ...
                  />
                  {getPhoneNumberOptions().length > 0 && (
                    <p className="text-xs text-gray-500 mt-1">
                      Select the phone number to use for sending WhatsApp messages to candidates.
                    </p>
                  )}
                </div>

                {/* WhatsApp Business Accounts info */}
                {whatsappAssets.whatsappAccounts.length > 0 && (
                  <div className="text-xs text-gray-500 p-3 bg-gray-50 rounded-lg">
                    <p className="font-medium text-gray-700 mb-1">Available WhatsApp Business Accounts:</p>
                    <ul className="space-y-1">
                      {whatsappAssets.whatsappAccounts.map((waba) => (
                        <li key={waba.id}>
                          {waba.name || waba.id} - {waba.phoneNumbers?.length || 0} phone number(s)
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  <button
                    onClick={loadWhatsappAssets}
                    disabled={whatsappAssetsLoading}
                    className="text-sm text-gray-600 hover:text-gray-700 font-medium flex items-center gap-1"
                  >
                    <RefreshCw
                      className={`w-4 h-4 ${
                        whatsappAssetsLoading ? "animate-spin" : ""
                      }`}
                    />
                    Refresh
                  </button>
                  <button
                    ...
                    disabled={!selectedPhoneNumber || (whatsappStatus?.connected && selectedPhoneNumber === whatsappStatus?.phoneNumberId)}
                    className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                      selectedPhoneNumber && (!whatsappStatus?.connected || selectedPhoneNumber !== whatsappStatus?.phoneNumberId)
