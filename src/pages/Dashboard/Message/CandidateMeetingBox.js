...
const ScheduledMeeting = ({ candidateData, close }) => {
  const {
    interviewMeetingTimezone,
    calendly_cancel_url,
    calendly_reschedule_url,
    interviewMeetingTimestamp,
    interviewMeetingTimestampEnd,
  } = candidateData;

  const openCalendlyCancelUrl = () => {
    window.open(calendly_cancel_url, "_blank");
    close();
  };

  const openCalendlyRescheduleUrl = () => {
    window.open(calendly_reschedule_url, "_blank");
    close();
  };

  return (
    <Card
      title="Scheduled Meeting Details"
      className="bg-white dark:bg-gray-900 rounded-lg shadow-lg dark:shadow-gray-400/50 hover:shadow-gray-600/50  mx-[-24px] my-[-30px]"
    >
      <p>
        <strong>Meeting Start Time:</strong>{" "}
        {moment(interviewMeetingTimestamp).format("HH:mm, Do MMM, YYYY")}
      </p>
      <p className="mb-5">
        <strong>Meeting End Time:</strong>{" "}
        {moment(interviewMeetingTimestampEnd).format("HH:mm, Do MMM, YYYY")}
      </p>
      {candidateData?.interviewMeetingLink && (
        <a
          className="mr-2 px-2 py-1 text-sm bg-indigo-500 text-white rounded"
          href={candidateData?.interviewMeetingLink}
          target="_blank"
        >
          Join Meeting
        </a>
      ...
      {/* Buttons to reschedule or cancel the event */}
      <div className="mt-8">
        <button
          className="mr-2 px-2 py-1 text-sm bg-indigo-500 text-white rounded"
          onClick={openCalendlyRescheduleUrl}
        >
          Reschedule Event
        </button>
        <button
          className="px-2 py-1 text-sm bg-red-500 text-white rounded"
          onClick={openCalendlyCancelUrl}
        >
          Cancel Event
        </button>
      </div>
...
const CandidateMeetingBox = ({
  ...
  const {
    ...
    defaultBody ?? localStorage["lastMeetingMessage_message"] ?? ""
  );
  const [includeBCC, setIncludeBcc] = useState(
    localStorage["lastMeetingMessage_includeBCC"] === "true"
  );
  ...
  if (
    candidateData?.meetingScheduled &&
    new Date(candidateData?.interviewMeetingTimestampEnd) > new Date()
  )
    return <ScheduledMeeting candidateData={candidateData} close={close} />;
  ...
  return (
    <>
      ...
      {candidateData?.cancelledMeeting && (
        <Alert
          type="warning"
          message={`The meeting previously arranged has been cancelled${
            candidateData?.cancelAt
              ? " at " +
                moment(candidateData?.cancelAt).format(STANDARD_MOMENT_FORMAT)
              : ""
          }${
            candidateData?.cancelledBy
              ? " by " + candidateData?.cancelledBy
              : ""
          }${
            candidateData?.cancelReason
              ? ". Reason: " + candidateData?.cancelReason
              : "."
          }`}
        />
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={async () => {
            ...
            if (!body.includes("@[Calendar Link](calendarLink)"))
            ...
            try {
              ...
              handleReset();
              localStorage["lastMeetingMessage_subject"] = subject;
              localStorage["lastMeetingMessage_message"] = body;
              localStorage["lastMeetingMessage_includeBCC"] = includeBCC;
            } catch (e) {
            } finally {
              setLoading(false);
            }
...
export default CandidateMeetingBox;
