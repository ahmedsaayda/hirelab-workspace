...
const getMultiJobTranslations = (language, companyName) => {
  const translations = {
    ...
    German: {
      ...
      findTheRole:
        "Finden Sie die Rolle, die zu Ihren Fähigkeiten und Ambitionen passt.",
      ...
      footerDescription:
        "Entdecken Sie unsere spannenden Jobangebote und bewerben Sie sich noch heute!",
    ...
    Dutch: {
      ...
      footerDescription:
        "Ontdek onze spannende vacatures en solliciteer vandaag nog!",
    ...
    French: {
      ...
      footerDescription:
        "Découvrez nos offres d'emploi passionnantes et postulez dès aujourd'hui !",
    ...
    Spanish: {
      ...
      findTheRole:
        "Encuentra el puesto que coincide con tus habilidades y ambiciones.",
      ...
      footerDescription:
        "¡Descubre nuestras emocionantes oportunidades de trabajo y aplica hoy!",
    ...
    Italian: {
      ...
      findTheRole:
        "Trova il ruolo che corrisponde alle tue competenze e ambizioni.",
      ...
      footerDescription:
        "Scopri le nostre entusiasmanti opportunità di lavoro e candidati oggi!",
    ...
    Portuguese: {
      ...
      findTheRole:
        "Encontre a função que combina com suas habilidades e ambições.",
      ...
      footerDescription:
        "Descubra nossas oportunidades de emprego e candidate-se hoje!",
    ...
    Polish: {
      ...
      findTheRole:
        "Znajdź stanowisko, które pasuje do Twoich umiejętności i ambicji.",
      ...
      footerDescription:
        "Odkryj nasze ekscytujące oferty pracy i aplikuj już dziś!",
...
const MultiJobCampaignModal = ({
  ...
  const generateAIContent = async () => {
    const selectedJobTitles = selectedCampaignObjects
      .map((c) => c.vacancyTitle)
      .join(", ");
    const departments = [
      ...new Set(
        selectedCampaignObjects.map((c) => c.department).filter(Boolean)
      ),
    ];
    const companyName =
      brandingDetails?.companyName || user?.companyName || "the company";
    ...
    const jobCategory =
      departments.length > 0 ? departments.join(", ") : "various roles";
    ...
    for (const campaign of selectedCampaignObjects) {
      ...
      if (campaign.aboutTheCompanyDescription?.trim()) {
        existingAboutContent.push(
          campaign.aboutTheCompanyDescription.substring(0, 200)
        );
      }
    }

    // Deduplicate facts by title
    const uniqueFacts = existingCompanyFacts
      .reduce((acc, fact) => {
        const titleLower = (fact.title || "").toLowerCase().trim();
        if (
          titleLower &&
          !acc.some((f) => (f.title || "").toLowerCase().trim() === titleLower)
        ) {
          acc.push(fact);
        }
        return acc;
...
`
    ...
    const languageMap = {
      English: {
        ...
        notLang: "Do NOT write in German, Dutch, or any other language",
      },
      German: {
        native: "Deutsch",
        instruction: "Schreiben Sie auf Deutsch",
        notLang:
          "Schreiben Sie NICHT auf Englisch, Niederländisch oder einer anderen Sprache",
      },
      Dutch: {
        native: "Nederlands",
        instruction: "Schrijf in het Nederlands",
        notLang:
          "Schrijf NIET in het Duits, Engels of een andere taal. Nederlands is NIET Duits!",
      },
      French: {
        native: "Français",
        instruction: "Écrivez en français",
        notLang: "N'écrivez PAS en allemand, anglais ou toute autre langue",
      },
      Spanish: {
        native: "Español",
        instruction: "Escribe en español",
        notLang: "NO escribas en alemán, inglés u otro idioma",
      },
      Italian: {
        native: "Italiano",
        instruction: "Scrivi in italiano",
        notLang: "NON scrivere in tedesco, inglese o in altre lingue",
      },
      Portuguese: {
        native: "Português",
        instruction: "Escreva em português",
        notLang: "NÃO escreva em alemão, inglês ou qualquer outro idioma",
      },
      Polish: {
        native: "Polski",
        instruction: "Pisz po polsku",
        notLang: "NIE pisz po niemiecku, angielsku ani w żadnym innym języku",
      },
      Turkish: {
        native: "Türkçe",
        instruction: "Türkçe yazın",
        notLang: "Almanca, İngilizce veya başka bir dilde YAZMAYIN",
      },
      Arabic: {
        native: "العربية",
        instruction: "اكتب بالعربية",
        notLang: "لا تكتب بالألمانية أو الإنجليزية أو أي لغة أخرى",
      },
      Chinese: {
        native: "中文",
        instruction: "请用中文写",
        notLang: "不要用德语、英语或其他任何语言书写",
      },
      Japanese: {
        native: "日本語",
        instruction: "日本語で書いてください",
        notLang: "ドイツ語、英語、または他の言語で書かないでください",
      },
    };
...
══════════════════════════════════════════════════════════════
This is a general careers page showcasing opportunities across ${
          departments.length > 0 ? departments.length : "multiple"
        } departments at ${companyName}.
...
1. heroTitle: ${
      isDepartmental
        ? `Inspiring headline for ${departmentName} careers`
        : "Inspiring headline for careers"
    } at ${companyName} (max 60 chars)
2. heroDescription: ${
      isDepartmental
        ? `Compelling paragraph about why ${departmentName} professionals should join - focus on department importance and impact`
        : "Compelling paragraph about why professionals should join"
    } (150-200 words)
...
7. aboutTheCompanyDescription: ${
      isDepartmental
        ? `Description focusing on how ${departmentName} department drives company success`
        : "Description of the company culture and mission"
    } (150-200 words)
8. companyFactsTitle: Title for Company Facts/Why Join Us section (max 40 chars)
9. companyFacts: Array of 4 compelling facts about working at ${companyName}${
      isDepartmental ? `, specifically for ${departmentName} roles` : ""
...
RESPOND IN JSON ONLY (values in ${selectedLanguage}):
{
  ...
  "candidateProcessSteps": [
    ...
    const response = await AiService.generateSectionContent({
      ...
      fields: [
        ...
        },
        {
          ...
          label: "Jobs Section Title",
          type: "text",
          maxLength: 40,
        },
        {
          key: "jobsSectionDescription",
          label: "Jobs Section Description",
          type: "text",
          maxLength: 150,
        },
        {
    ...
    if (typeof result.companyFacts === "string") {
      try {
        result.companyFacts = JSON.parse(result.companyFacts);
      } catch (e) {
        result.companyFacts = [];
      }
    }
    if (typeof result.candidateProcessSteps === "string") {
      try {
        result.candidateProcessSteps = JSON.parse(result.candidateProcessSteps);
      } catch (e) {
        result.candidateProcessSteps = [];
      }
    }
    ...
    try {
      ...
      const response = await AiService.createMultiVacanciesPage({
        ...
        brandingDetails: {
          ...
          secondaryColor:
            brandingDetails?.secondaryColor || user?.secondaryColor,
      ...
      if (response?.data?.success && response?.data?.data?.campaign) {
        ...
        console.log(
          "Multi-job campaign created successfully via backend:",
          newCampaign._id
        );
  ...
  const createCampaignWithContent = async (generatedContent = {}) => {
    const companyName =
      brandingDetails?.companyName || user?.companyName || "Our Company";
    ...
    const generatedJobsSectionDescription =
      generatedContent.jobsSectionDescription || "";
    ...
    const generatedAboutDescription =
      generatedContent.aboutTheCompanyDescription || "";
    const generatedFactsTitle = generatedContent.companyFactsTitle || "";
    let generatedFacts = Array.isArray(generatedContent.companyFacts)
      ? generatedContent.companyFacts
      : [];
    const generatedProcessTitle = generatedContent.candidateProcessTitle || "";
    const generatedProcessSteps = Array.isArray(
      generatedContent.candidateProcessSteps
    )
      ? generatedContent.candidateProcessSteps
      : [];
    ...
    if (generatedFacts.length === 0 && selectedCampaignObjects.length > 0) {
      console.log(
        "AI didn't generate facts, checking linked campaigns for existing facts..."
      );
      for (const campaign of selectedCampaignObjects) {
        if (
          Array.isArray(campaign.companyFacts) &&
          campaign.companyFacts.length > 0
        ) {
          // Check if any fact has actual content (headingText or descriptionText)
          const validFacts = campaign.companyFacts.filter(
            (f) =>
              (f.headingText && f.headingText.trim()) ||
              (f.descriptionText && f.descriptionText.trim())
          );
          if (validFacts.length > 0) {
            console.log(
              `Using ${validFacts.length} facts from linked campaign:`,
              campaign.vacancyTitle
            );
    ...
    try {
      ...
      console.log(
        `MultiJobCampaignModal: Searching Unsplash for hero with query: "${heroSearchQuery}"`
      );

      const heroImageResponse = await AiService.searchUnsplash(
        heroSearchQuery,
        6
      );

      if (
        heroImageResponse?.data?.success &&
        heroImageResponse.data.data?.length > 0
      ) {
        const randomIndex = Math.floor(
          Math.random() * Math.min(heroImageResponse.data.data.length, 6)
        );
        const selectedImage = heroImageResponse.data.data[randomIndex];

        console.log(
          "MultiJobCampaignModal: Uploading hero image to Cloudinary..."
        );
        ...
        console.log(
          "MultiJobCampaignModal: No hero images found from Unsplash, heroImage will be empty"
        );
      }
    } catch (imgError) {
      console.error(
        "MultiJobCampaignModal: Failed to fetch/upload hero image:",
        imgError
      );
    }
    ...
    try {
      ...
      console.log(
        `MultiJobCampaignModal: Searching Unsplash for About section with query: "${aboutSearchQuery}"`
      );

      const aboutImageResponse = await AiService.searchUnsplash(
        aboutSearchQuery,
        8
      );

      if (
        aboutImageResponse?.data?.success &&
        aboutImageResponse.data.data?.length > 0
      ) {
        const images = aboutImageResponse.data.data.slice(0, 4); // Get up to 4 images

        // Upload each image to Cloudinary
        console.log(
          `MultiJobCampaignModal: Uploading ${images.length} About section images...`
        );
        ...
        console.log(
          `MultiJobCampaignModal: Uploaded ${aboutTheCompanyImages.length} About section images`
        );
      }
    } catch (imgError) {
      console.error(
        "MultiJobCampaignModal: Failed to fetch/upload About section images:",
        imgError
      );
    }
    ...
    const campaignData = {
      ...
      aboutTheCompanyDescription:
        generatedAboutDescription || t.greatWorkHappens,
      ...
      companyFacts:
        generatedFacts.length > 0
          ? generatedFacts.map((fact, idx) => ({
              icon:
                fact.icon || ["star", "users", "trending-up", "heart"][idx % 4],
      ...
      candidateProcessSteps:
        generatedProcessSteps.length > 0
          ? generatedProcessSteps.map((step, idx) => ({
    ...
    <div className="space-y-5">
              ...
              >
                <div className="flex items-center gap-3.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div
                      className={`flex flex-wrap items-center gap-x-3 gap-y-1 mt-0.5 text-xs ${
      ...
      <div className="flex items-start gap-3 p-3.5 rounded-xl bg-amber-50/60 border border-amber-100/80">
        ...
        <p className="text-xs text-amber-700">
          Selected jobs will appear as cards on your career page. Visitors can
      ...
      <div className="p-8">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Showcase multiple positions</p>
          </div>
        </div>
